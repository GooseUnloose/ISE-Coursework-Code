...
def train(id,phase,inpCSV,seeds):
...
def logTest(id,model,phase,freq,accuracy,precision,recall,f1_score,aucValues,processTime):
    ...
    if phase == False:
        out_csv_name = f'C:/Users/Goose/Desktop/Results/1Phase_{id}_{model}'
        
    else:
        out_csv_name = f'C:/Users/Goose/Desktop/2Phase_{id}_{model}'
...
def dfCycle():
    ...
    for i in datasets:
        print(f"\n\n{i}\n")
        df = pd.read_csv(f"C:/Users/Goose/Downloads/{i}.csv").fillna("")
        df = dfReduction(df)
